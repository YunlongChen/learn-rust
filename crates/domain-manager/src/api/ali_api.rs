use crate::api::model::dns_operate::{DnsOperateResponse, DnsRecordDeleteResponse, RecordLog};
use crate::api::model::domain::{Domain, DomainQueryResponse};
...
use domain_clients::{call_api, RequestBody};
...
pub fn process_domain_data(data: &Vec<Domain>) -> Result<Vec<DomainName>, TryReserveError> {
...
pub async fn add_aliyun_dns_record(add_dns_field: &AddDnsField) -> bool {
    ...
    let query_params = &[
        ("RegionId", "cn-qingdao"),
        ("DomainName", add_dns_field.domain_name.as_str()),
        ("RR", &add_dns_field.record_name),
        ("Type", value.get_value()),
        ("Value", &add_dns_field.value),
    ];
    // 构建查询参数  InstanceId的在元数据中显示的类型是array，"in":"query"，非必填
    // let region_id = "cn-hangzhou";
    // let instance_ids = vec![
    ...
    body.insert(
        ...
        json!(&add_dns_field.domain_name),
    );
    body.insert(String::from("RR"), json!(&add_dns_field.record_name));

    let value = &add_dns_field.clone().record_type.unwrap();
    body.insert(String::from("Type"), json!(value));
    body.insert(String::from("Value"), json!(&add_dns_field.value));
