...
impl AliyunDnsApi {
    ...
    /// 调用阿里云API
    async fn call_aliyun_api(
        &self,
        action: &str,
        params: HashMap<String, String>,
    ) -> Result<Value, DnsApiError> {
    ...
    fn parse_single_record(&self, record_value: &Value) -> Result<Record, DnsApiError> {
        ...
        let record_type_str = record_value
            .get("Type")
            .and_then(|v| v.as_str())
            .unwrap_or("A");
        ...
        Ok(Record::new(status, rr, record_type, value, record_id, ttl))
    }
}
...
impl DnsApiTrait for AliyunDnsApi {
    ...
    async fn query_dns_records(
        ...
        // 调用阿里云API
        let response = self
            .call_aliyun_api("DescribeDomainRecords", params)
            .await?;
    ...
    async fn get_dns_record(&self, record_id: &str) -> Result<Record, DnsApiError> {
        ...
        // 调用阿里云API
        let response = self
            .call_aliyun_api("DescribeDomainRecordInfo", params)
            .await?;
    ...
    async fn create_dns_record(
        ...
        // 验证记录值格式
        DnsRecordValidator::validate_record_by_type(
            &request.record_type,
            &request.value,
            request.priority,
        )?;
    ...
    async fn batch_delete_dns_records(
        ...
        request.validate().map_err(|e| {
            DnsApiError::ValidationError(format!("批量删除请求参数验证失败: {}", e))
        })?;

        info!(
            "批量删除DNS记录: 域名={}, 主机记录={}, 类型={:?}",
        ...
        // 调用阿里云API
        let response = self
            .call_aliyun_api("DeleteSubDomainRecords", params)
            .await?;
...
impl AliyunDnsApi {
    /// 查询域名列表
    pub async fn query_domain_list(
        &self,
    ...
    /// 查询DNS操作日志
    pub async fn query_dns_operation_logs(
        &self,
        domain_name: &str,
        page_number: Option<u32>,
        page_size: Option<u32>,
        ...
        info!(
            "查询DNS操作日志: 域名={}, 页码={:?}, 每页={:?}",
            domain_name, page_number, page_size
        );
    ...
    pub async fn set_dns_record_status(
        ...
        params.insert(
            "Status".to_string(),
            if status { "Enable" } else { "Disable" }.to_string(),
        );

        let response = self
            .call_aliyun_api("SetDomainRecordStatus", params)
            .await?;
    ...
    /// 获取DNS记录解析统计
    pub async fn get_dns_record_statistics(
        &self,
        domain_name: &str,
        start_date: &str,
        end_date: &str,
        ...
        let response = self
            .call_aliyun_api("DescribeDomainStatistics", params)
            .await?;
...
mod tests {
    ...
    fn test_with_endpoint() {
        let client = create_test_client().with_endpoint("https://custom.endpoint.com".to_string());
    ...
    async fn test_validate_dns_record() {
        ...
        // 测试CNAME记录验证
        assert!(client
            .validate_dns_record(&Type::Cname, "example.com")
            .is_ok());
