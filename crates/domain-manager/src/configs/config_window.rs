...
#[cfg(not(test))]
use crate::{location, DOMAIN_MANAGER_LOWERCASE};
...
impl ConfigWindow {
    ...
    #[cfg(not(test))]
    pub fn load() -> Self {
        if let Ok(window) = confy::load::<ConfigWindow>(DOMAIN_MANAGER_LOWERCASE, Self::FILE_NAME) {
            window
        } else {
            let _ = confy::store(
...
#[cfg(test)]
mod tests {
    use crate::configs::config_window::ConfigWindow;
