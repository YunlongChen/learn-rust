...
pub fn add_record(
    ...
    conn.execute(
        "INSERT INTO domain_records (account_id, domain_id, record_name, record_type, value, ttl) VALUES (?1, ?2, ?3, ?4, ?5, ?6);",
        params![domain_entity.account_id,domain_entity.id, new_record.record_name,new_record.record_type,new_record.record_value,new_record.ttl],
    )?;
...
/// 获取用户的所有域名
pub fn get_account_domains(conn: &Connection, domain_id: Option<i64>) -> Result<Vec<RecordEntity>> {
