...
use reqwest::Client;
use crate::gui::model::domain::{DomainName, Domain, DnsProvider};
...
impl CloudflareDnsClient {
    ...
    fn convert_internal_to_cf_record(record: &Record) -> CloudflareDnsRecord {
        ...
        CloudflareDnsRecord {
            id: if record.record_id.is_empty() { None } else { Some(record.record_id.clone()) },
...
impl DnsClientTrait for CloudflareDnsClient {
    ...
    fn update_dns_record(&self, domain_name: &DomainName, record: &Record) -> Result<()> {
        ...
        rt.block_on(async {
            ...
            // 更新DNS记录
            let url = format!("{}/zones/{}/dns_records/{}", self.base_url, zone_id, record.record_id);
            let response = self.client
                .put(&url)
                .header("Authorization", format!("Bearer {}", self.api_token))
                .header("Content-Type", "application/json")
