...
impl DnsApiTrait for DnsClient {
    ...
    async fn query_dns_records(
        ...
        match self {
            DnsClient::Aliyun(client) => client.query_dns_records(query).await,
        }
    }
    ...
    async fn get_dns_record(&self, record_id: &str) -> Result<Record, DnsApiError> {
        match self {
            DnsClient::Aliyun(client) => client.get_dns_record(record_id).await,
        }
    }
    ...
    async fn create_dns_record(
        ...
        match self {
            DnsClient::Aliyun(client) => client.create_dns_record(request).await,
        }
    }
    ...
    async fn update_dns_record(
        ...
        match self {
            DnsClient::Aliyun(client) => client.update_dns_record(request).await,
        }
    }
    ...
    async fn delete_dns_record(
        ...
        match self {
            DnsClient::Aliyun(client) => client.delete_dns_record(request).await,
        }
    }
    ...
    async fn batch_delete_dns_records(
        ...
        match self {
            DnsClient::Aliyun(client) => client.batch_delete_dns_records(request).await,
        }
    }
    ...
    fn validate_dns_record(&self, record_type: &Type, value: &str) -> Result<(), DnsApiError> {
        match self {
            DnsClient::Aliyun(client) => client.validate_dns_record(record_type, value),
        }
    }
}
...
impl DnsApiManager {
    ...
    pub async fn create_dns_record(
        ...
        info!(
            "创建DNS记录: 域名={}, 主机记录={}",
            request.domain_name, request.rr
        );
    ...
    /// 验证DNS记录
    pub async fn validate_dns_record(
        &self,
        record_type: &crate::model::dns_record_response::Type,
        value: &str,
    ) -> Result<(), DnsApiError> {
        let client_arc = self
            .get_client()
...
impl DnsApiFactory {
    ...
    /// 创建默认的阿里云DNS API管理器
    pub async fn create_aliyun_manager(
        access_key_id: String,
        access_key_secret: String,
    ) -> Result<DnsApiManager> {
        let config = DnsApiConfig {
            provider: DnsProvider::Aliyun,
            access_key_id,
            access_key_secret,
...
mod tests {
    ...
    fn test_dns_provider_display() {
        ...
        assert_eq!(
            DnsProvider::Other("Custom".to_string()).to_string(),
            "Custom"
        );
    }
