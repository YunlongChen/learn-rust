...
impl DnsRecordValidator {
    ...
    pub fn validate_cname_record(value: &str) -> Result<(), DnsApiError> {
        if value.is_empty() {
            return Err(DnsApiError::ValidationError("CNAME记录值不能为空".to_string()));
        }
        
        // 简单的域名格式验证
        if !value.chars().all(|c| c.is_alphanumeric() || c == '.' || c == '-') {
            return Err(DnsApiError::ValidationError(format!("无效的CNAME记录值: {}", value)));
        }
    ...
    pub fn validate_mx_record(value: &str, priority: Option<u32>) -> Result<(), DnsApiError> {
        ...
        if priority.is_none() {
            return Err(DnsApiError::ValidationError("MX记录必须指定优先级".to_string()));
        }
        
        let priority = priority.unwrap();
        if !(1..=50).contains(&priority) {
            return Err(DnsApiError::ValidationError("MX记录优先级必须在1-50之间".to_string()));
        }
    ...
    pub fn validate_txt_record(value: &str) -> Result<(), DnsApiError> {
        if value.is_empty() {
            return Err(DnsApiError::ValidationError("TXT记录值不能为空".to_string()));
        }
        
        // TXT记录长度限制（通常为255字符）
        if value.len() > 255 {
            return Err(DnsApiError::ValidationError("TXT记录值长度不能超过255字符".to_string()));
        }
    ...
    pub fn validate_rr(rr: &str) -> Result<(), DnsApiError> {
        ...
        // 主机记录格式验证
        if rr != "@" && !rr.chars().all(|c| c.is_alphanumeric() || c == '-' || c == '_') {
            return Err(DnsApiError::ValidationError(format!("无效的主机记录格式: {}", rr)));
        }
    ...
    pub fn validate_ttl(ttl: u32) -> Result<(), DnsApiError> {
        ...
        if !(1..=86400).contains(&ttl) {
            return Err(DnsApiError::ValidationError("TTL值必须在1-86400秒之间".to_string()));
        }
    ...
    /// 根据记录类型验证记录值
    pub fn validate_record_by_type(record_type: &Type, value: &str, priority: Option<u32>) -> Result<(), DnsApiError> {
        match record_type {
...
impl DnsApiUtils {
    ...
    pub fn build_query_params(query: &DnsRecordQuery) -> HashMap<String, String> {
        ...
        if let Some(ref status) = query.status {
            params.insert("Status".to_string(), match status {
    ...
    pub fn build_create_params(request: &CreateDnsRecordRequest) -> HashMap<String, String> {
        ...
        params.insert("Type".to_string(), request.record_type.get_value().to_string());
    ...
    pub fn parse_api_error(error_code: &str, error_message: &str) -> DnsApiError {
        match error_code {
            ...
            "Forbidden.RAM" | "Forbidden.Risk" => DnsApiError::PermissionDenied(error_message.to_string()),
    ...
    pub fn log_operation(operation: &str, domain: &str, record_id: Option<&str>, success: bool) {
        if success {
            info!("DNS操作成功: {} - 域名: {}, 记录ID: {:?}", operation, domain, record_id);
        } else {
            error!("DNS操作失败: {} - 域名: {}, 记录ID: {:?}", operation, domain, record_id);
        }
    }
}
