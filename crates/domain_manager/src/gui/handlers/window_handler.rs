...
use super::message_handler::WindowMessage;
use crate::gui::state::{AppState, StateUpdate, UiUpdate};
use iced::{Point, Size};
...
impl WindowHandler {
    ...
    /// 处理开始拖拽窗口
    fn handle_start_drag(&mut self, state: &mut AppState, position: Point) -> HandlerResult {
    ...
    fn handle_window_minimize(&mut self, state: &mut AppState) -> HandlerResult {
        state.ui.set_message("窗口已最小化".to_string());
    ...
    /// 处理窗口最大化/还原
    fn handle_window_maximize(&mut self, state: &mut AppState, is_maximized: bool) -> HandlerResult {
        ...
        state.ui.is_window_maximized = is_maximized;
    ...
    fn handle_toggle_floating(&mut self, state: &mut AppState) -> HandlerResult {
        let new_floating_mode = !state.ui.is_floating_mode;
        state.ui.is_floating_mode = new_floating_mode;
    ...
    /// 处理背景透明度变更
    fn handle_background_opacity_change(&mut self, state: &mut AppState, opacity: f32) -> HandlerResult {
        // 限制透明度范围
    ...
    pub fn get_window_state(&self, state: &AppState) -> WindowState {
        if state.ui.is_floating_mode {
            WindowState::Floating
        } else if state.ui.is_window_maximized {
    ...
    pub fn save_window_settings(&self, state: &AppState) -> Result<(), String> {
        ...
        let settings = serde_json::json!({
            "position": self.last_window_position,
            "size": self.last_window_size,
            "is_maximized": state.ui.is_window_maximized,
            "is_floating_mode": state.ui.is_floating_mode,
    ...
    pub fn load_window_settings(&mut self, state: &mut AppState) -> Result<(), String> {
        ...
        state.ui.is_window_maximized = false;
        state.ui.is_floating_mode = false;
...
impl EventHandler<WindowMessage> for WindowHandler {
    fn handle(&self, state: &mut AppState, event: WindowMessage) -> HandlerResult {
        ...
        match event {
            WindowMessage::StartDrag(position) => {
                // 这里需要可变引用，实际实现时需要调整
                HandlerResult::StateUpdated
            },
            ...
            WindowMessage::WindowMoved(position) => {
                HandlerResult::StateUpdated
            },
            ...
            WindowMessage::ToggleFloating => {
                HandlerResult::StateUpdated
            },
            WindowMessage::BackgroundOpacityChange(opacity) => {
                HandlerResult::StateUpdated
            },
            WindowMessage::BackgroundToggle => {
                HandlerResult::StateUpdated
            },
    ...
    fn can_handle(&self, event: &WindowMessage) -> bool {
        // 窗口处理器可以处理所有窗口相关的消息
        true
    }
}
...
impl MutableWindowHandler {
    ...
    pub fn handle(&self, state: &mut AppState, event: WindowMessage) -> HandlerResult {
        ...
        match event {
            WindowMessage::StartDrag(position) => {
                handler.handle_start_drag(state, position)
            },
            WindowMessage::DragWindow(position) => {
                handler.handle_drag_window(state, position)
            },
            WindowMessage::WindowMoved(position) => {
                handler.handle_window_moved(state, position)
            },
            WindowMessage::WindowResized(size) => {
                handler.handle_window_resized(state, size)
            },
            WindowMessage::WindowMinimize => {
                handler.handle_window_minimize(state)
            },
            WindowMessage::WindowMaximize(is_maximized) => {
                handler.handle_window_maximize(state, is_maximized)
            },
            WindowMessage::ToggleFloating => {
                handler.handle_toggle_floating(state)
            },
            WindowMessage::BackgroundOpacityChange(opacity) => {
                handler.handle_background_opacity_change(state, opacity)
            },
            WindowMessage::BackgroundToggle => {
                handler.handle_background_toggle(state)
            },
            WindowMessage::CloseRequest => {
                handler.handle_window_close_request(state)
            },
        }
    }
