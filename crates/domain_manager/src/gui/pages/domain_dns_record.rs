...
use iced::widget::{button, pick_list, slider, text, text_input, Column, Container, Row, Text};
use iced::{Alignment, Length};

pub fn dns_record<'a>(app: &DomainManager) -> Container<'a, Message, StyleType> {
    // // 展示dns列表
    // let page = match &app.current_domain_name {
    //     // 选中了域名
    //     Some(domain_name) => {
    //         // 返回到解析界面
    ...
    //     }
    // };
    let container = Column::new().spacing(20);
