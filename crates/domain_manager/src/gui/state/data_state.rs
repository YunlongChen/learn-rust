...
pub struct DataState {
    ...
    /// DNS记录缓存 (域名 -> DNS记录列表)
    pub dns_records_cache: HashMap<String, Vec<DnsRecord>>,
    
    /// 当前显示的DNS记录
    pub current_dns_records: Vec<DnsRecord>,
...
impl DataState {
    ...
    pub fn remove_domain(&mut self, domain_name: &str) {
        self.domain_list.retain(|d| d.name != domain_name);
        self.dns_records_cache.remove(domain_name);
        
        // 如果删除的是当前选中的域名，清除选择
        if let Some(selected) = &self.selected_domain {
            if selected.name == domain_name {
    ...
    pub fn select_domain(&mut self, domain: Domain) {
        // 从缓存中加载DNS记录
        if let Some(records) = self.dns_records_cache.get(&domain.name) {
    ...
    /// 设置DNS记录
    pub fn set_dns_records(&mut self, domain_name: String, records: Vec<DnsRecord>) {
        self.dns_records_cache.insert(domain_name.clone(), records.clone());
    ...
    /// 添加DNS记录
    pub fn add_dns_record(&mut self, domain_name: String, record: DnsRecord) {
        let records = self.dns_records_cache.entry(domain_name.clone()).or_insert_with(Vec::new);
    ...
    /// 删除DNS记录
    pub fn remove_dns_record(&mut self, domain_name: &str, record_id: &str) {
        if let Some(records) = self.dns_records_cache.get_mut(domain_name) {
        ...
        if let Some(selected) = &self.selected_domain {
            if selected.name == domain_name {
                self.current_dns_records.retain(|r| r.record_id != record_id);
            }
        }
    ...
    pub fn get_filtered_domains(&self) -> Vec<Domain> {
        ...
        if let Some(provider) = &self.filter.provider {
            filtered.retain(|d| d.provider.as_ref().map_or(false, |p| p == provider));
        }
