...
```bash
# 克隆代码后，进入 domain_stun 目录
cd crates/domain_stun

# 编译 release 版本
cargo build --release

# 二进制文件位于
# target/release/domain_stun
```
...
```bash
# 创建数据目录
sudo mkdir -p /opt/domain_stun/data
sudo mkdir -p /var/log/domain_stun

# 创建配置目录
sudo mkdir -p /etc/domain_stun
```
...
```bash
sudo nano /etc/domain_stun/config.toml
```
...
[database]
url = "sqlite:/opt/domain_stun/data/domain_stun.db"
```

### 4. 设置权限

```bash
sudo chown -R $USER:$USER /opt/domain_stun
sudo chmod -R 755 /opt/domain_stun
```
...
```bash
sudo nano /etc/systemd/system/domain_stun.service
```
...
Type=simple
User=root
WorkingDirectory=/opt/domain_stun
ExecStart=/opt/domain_stun/domain_stun
Restart=always
RestartSec=5
...
sudo systemctl enable domain_stun
sudo systemctl start domain_stun

# 查看状态
sudo systemctl status domain_stun
```
...
# 构建 Docker 镜像
docker build -t domain_stun:latest .
```
...
services:
  domain_stun:
    image: domain_stun:latest
    container_name: domain_stun
...
```bash
sudo journalctl -u domain_stun -f
```
...
# 导出日志到文件
docker compose logs > domain_stun.log
```
...
```bash
# 直接部署
cp /opt/domain_stun/data/domain_stun.db /path/to/backup/domain_stun.db.$(date +%Y%m%d)

# Docker 部署
docker compose exec domain_stun sh -c 'cp /data/domain_stun.db /data/backup.db'
docker cp domain_stun:/data/backup.db ./domain_stun.db.$(date +%Y%m%d)
```
...
```bash
# 停止服务
sudo systemctl stop domain_stun
...
# 重启服务
sudo systemctl restart domain_stun
```
...
```bash
# 查看详细日志
docker compose logs domain_stun

# 检查容器状态
docker ps -a | grep domain_stun
```
...
- [domain_agent README](../domain_agent/README.md) - Agent 端使用说明
